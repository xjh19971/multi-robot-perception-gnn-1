...
def cal_relative_pose(p1, p2):
    ...
    # q2 * q1.inverse
    rp = quat_to_so3([(w1 * x2 + x1 * w2 + y1 * z2 - z1 * y2), (w1 * y2 - x1 * z2 + y1 * w2 + z1 * x2),
                      (w1 * z2 + x1 * y2 - y1 * x2 + z1 * w2), (w1 * w2 - x1 * x2 - y1 * y2 - z1 * z2)])[:6]
    return np.concatenate((p2t - p1t, rp))


if __name__ == '__main__':
    print(cal_relative_pose(np.array(
        [-5.902101516723632812e+01, 1.927141571044921875e+02, 5.996415138244628906e+00, -2.683681845664978027e-01,
         2.737782299518585205e-01, 6.468879580497741699e-01, 6.592115163803100586e-01]), np.array(
        [-5.506656265258789062e+01, 1.961531982421875000e+02, 5.998308658599853516e+00, -4.198011383414268494e-02,
         3.792373836040496826e-01, 1.026619002223014832e-01, 9.186278581619262695e-01])))
